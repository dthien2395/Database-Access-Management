...
//	}
	...
	public static <T> int dropTable(DatabaseType databaseType, Class<T> dataClass,
									boolean ignoreErrors) throws SQLException {
		TableInfo<T> tableInfo = new TableInfo<T>(databaseType, DatabaseTableConfig.fromClass(databaseType, dataClass));
		Collection<String> statements = dropTableStatements(databaseType, tableInfo);
		int stmtC = 0;
		Connection connection = MyDatabaseConnection.getConnection();
		for (String statement : statements) {
